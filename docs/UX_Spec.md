...
## Part V: Admin Dashboard (Web)

The Admin Dashboard is the **operations interface** for Vigor. It is not for users—it is for operators who maintain the Ghost.

### 5.1 Design Philosophy: Visibility Without Vanity

> _"The Admin Dashboard exists so the Ghost can remain invisible to users."_

While the iOS app and Watch follow the invisibility principle, the Admin Dashboard inverts this completely. **Operators need to see everything the Ghost is doing.**

| User Interface   | Admin Interface      |
| ---------------- | -------------------- |
| Minimal UI       | Rich dashboards      |
| Hide complexity  | Expose all decisions |
| Silent operation | Verbose logging      |
| Trust the Ghost  | Verify the Ghost     |

**The Admin Dashboard is a telescope into the Ghost's mind.** Every decision, every transformation, every safety breaker trigger must be visible and auditable.

### 5.2 Admin Voice & Tone

The Admin Dashboard follows a **clinical, precise** tone—different from the Ghost's concierge voice:

| ❌ Never (Admin UI)          | ✅ Always (Admin UI)            |
| ---------------------------- | ------------------------------- |
| "Everything looks great! 🎉" | "3/3 services operational"      |
| "Users are loving Vigor!"    | "1,247 active users (7d)"       |
| "AI is working hard!"        | "gpt-5-mini: 145ms avg latency" |
| Colorful celebration         | Neutral status indicators       |

**Tone characteristics:**

- **Clinical.** Report facts, not feelings.
- **Precise.** Specific numbers, timestamps, percentages.
- **Neutral.** Green/yellow/red for status, not "great/okay/bad."
- **Audit-ready.** Every screen should be screenshottable for compliance.

### 5.3 Access Control

**Admin access is explicitly granted, never inferred.**
...
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  Admin list maintained in:                                                  │
│  • frontend/src/config/adminConfig.ts (frontend gating)                     │
│  • functions-modernized/shared/config.py (backend enforcement)              │
│                                                                             │
│  Authorization flow:                                                        │
│  1. User authenticates via Microsoft Entra ID                               │
│  2. Frontend checks email against admin whitelist                           │
│  3. Backend re-validates on every admin API call                            │
│  4. Unauthorized access returns 403 (no data leakage)                       │
│                                                                             │
│  Initial Admins:                                                            │
...
**Non-admins who navigate to /admin see:**
...
│  ─────────────────────────────────────────────────────────────              │
│                                                                             │
│    🏠 Dashboard          Overview, Ghost status, key metrics                │
│    👥 Users              User management with Trust phases                  │
│    👻 Ghost Health       Ghost system status, degradation state             │
│    🤖 AI Pipeline        RAG + gpt-5-mini monitoring                        │
│    📊 Analytics          Ghost operations metrics (not user fitness)        │
│    🔒 Audit              Decision Receipts, Safety Breakers                 │
│    📦 Bulk Ops           Batch user operations                              │
...
The Dashboard provides at-a-glance Ghost system health:

```
┌─────────────────────────────────────────────────────────────────────────────┐
│  Vigor Admin                                            Vedprakash Mishra   │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  GHOST STATUS                                                               │
│  ─────────────────────────────────────────────────────────────              │
│                                                                             │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐    │
│  │ Ghost Mode   │  │ Database     │  │ AI Pipeline  │  │ Active Users │    │
│  │              │  │              │  │              │  │              │    │
│  │  NORMAL ●    │  │ Connected ●  │  │ Healthy ●    │  │    1,247     │    │
│  │              │  │ Cosmos DB    │  │ gpt-5-mini   │  │   (7 days)   │    │
│  └──────────────┘  └──────────────┘  └──────────────┘  └──────────────┘    │
│                                                                             │
│                                                                             │
│  TRUST DISTRIBUTION                                                         │
...
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │                                                                     │   │
│  │  Observer        ████████████████████████  45%  (561 users)        │   │
│  │  Scheduler       ██████████████████        31%  (387 users)        │   │
│  │  Auto-Scheduler  ████████████              18%  (225 users)        │   │
│  │  Transformer     ████                       5%  (62 users)         │   │
│  │  Full Ghost      █                          1%  (12 users)         │   │
│  │                                                                     │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                                                             │
│  Avg. time to Phase 2: 8.3 days  │  Avg. time to Phase 5: 67 days         │
│                                                                             │
│                                                                             │
│  GHOST DECISIONS (24h)                                                      │
│  ─────────────────────────────────────────────────────────────              │
│                                                                             │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐    │
│  │ Decisions    │  │ Mutations    │  │ Accept Rate  │  │ Safety       │    │
│  │              │  │              │  │              │  │ Breakers     │    │
│  │    2,847     │  │    1,234     │  │   89.2%      │  │      3       │    │
│  │   (+12%)     │  │  calendar    │  │ (target 85%) │  │  triggered   │    │
│  └──────────────┘  └──────────────┘  └──────────────┘  └──────────────┘    │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

### 5.6 User Management with Trust Visibility

User table must expose Ghost-specific data:
...
│                                                                             │
│  ┌────────────────────────────────────────────────────────────────────────┐│
│  │ User          │ Tier    │ Trust    │ Score │ Watch │ Phenome │ Actions ││
│  ├───────────────┼─────────┼──────────┼───────┼───────┼─────────┼─────────┤│
│  │ John Doe      │ premium │ Auto-    │  74%  │  ●    │ Fresh   │ View    ││
│  │ john@...      │         │ Scheduler│       │       │         │         ││
│  ├───────────────┼─────────┼──────────┼───────┼───────┼─────────┼─────────┤│
│  │ Jane Smith    │ free    │ Observer │  23%  │  ○    │ Stale   │ View    ││
│  │ jane@...      │         │          │       │       │ (4d)    │         ││
│  ├───────────────┼─────────┼──────────┼───────┼───────┼─────────┼─────────┤│
│  │ Bob Wilson    │ enterp. │ Full     │  94%  │  ●    │ Fresh   │ View    ││
│  │ bob@...       │         │ Ghost    │       │       │         │         ││
│  └────────────────────────────────────────────────────────────────────────┘│
│                                                                             │
│  Legend:                                                                    │
│  • Trust Phase: Observer → Scheduler → Auto-Scheduler → Transformer → Full │
│  • Trust Score: 0-100% based on suggestion acceptance                       │
│  • Watch: ● Connected / ○ Not connected / ◐ Degraded                        │
│  • Phenome: Fresh (<6h) / Stale (age) / Missing                            │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

### 5.7 Ghost Health Monitor

Replaces generic "LLM Health" with Ghost-specific monitoring:

```
┌─────────────────────────────────────────────────────────────────────────────┐
│  Ghost Health Monitor                                          [Refresh]   │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  SYSTEM STATUS                                                              │
│  ─────────────────────────────────────────────────────────────              │
│                                                                             │
│  Ghost Mode: NORMAL ●                                                       │
│  Auto-degrades to SAFE MODE if >5% decision failures                       │
│                                                                             │
│                                                                             │
│  COMPONENT HEALTH                                                           │
│  ─────────────────────────────────────────────────────────────              │
│                                                                             │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │ Component          │ Status  │ Latency │ Error Rate │ Details      │   │
│  ├────────────────────┼─────────┼─────────┼────────────┼──────────────┤   │
│  │ RAG Retrieval      │    ●    │   45ms  │    0.1%    │ Exercise DB  │   │
│  │ gpt-5-mini         │    ●    │  180ms  │    0.3%    │ Structured   │   │
│  │ Workout Contracts  │    ●    │   12ms  │    0.0%    │ Validator    │   │
│  │ Cosmos DB          │    ●    │   23ms  │    0.0%    │ Connected    │   │
│  │ APNs (Silent Push) │    ●    │   89ms  │    1.2%    │ Morning orch │   │
│  │ CloudKit Sync      │    ●    │  156ms  │    0.8%    │ Phenome sync │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                                                             │
│                                                                             │
│  PHENOME STORES                                                             │
...
│                                                                             │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │ Store              │ Status  │ Size    │ Sync Rate  │ Staleness    │   │
│  ├────────────────────┼─────────┼─────────┼────────────┼──────────────┤   │
│  │ RawSignal          │    ●    │  2.3GB  │   99.2%    │ <2h avg      │   │
│  │ DerivedState       │    ●    │  890MB  │   98.7%    │ <4h avg      │   │
│  │ BehavioralMemory   │    ●    │  1.1GB  │   99.4%    │ <1h avg      │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

### 5.8 AI Pipeline Configuration

Replaces invalid LLM parameters with actual Ghost AI pipeline monitoring:
...
│                                                                             │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │  gpt-5-mini (Structured Outputs)                               ●   │   │
│  │  via Azure OpenAI • vigor-openai resource                          │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                                                             │
│  Note: gpt-5-mini uses structured output mode.                             │
│  Temperature, top_p, and penalty parameters are not applicable.            │
│                                                                             │
│                                                                             │
│  GENERATION PIPELINE                                                        │
│  ─────────────────────────────────────────────────────────────              │
│                                                                             │
│  ┌──────────────┐    ┌──────────────┐    ┌──────────────┐                  │
│  │    RAG       │ →  │  gpt-5-mini  │ →  │   Workout    │                  │
│  │  Retrieval   │    │  Generation  │    │   Contract   │                  │
│  │   45ms avg   │    │   180ms avg  │    │  Validator   │                  │
│  └──────────────┘    └──────────────┘    └──────────────┘                  │
│                                                                             │
│                                                                             │
│  WORKOUT CONTRACT VALIDATION (24h)                                          │
│  ─────────────────────────────────────────────────────────────              │
│                                                                             │
│  • Workouts generated:      1,245                                           │
│  • Contract violations:        12  (0.96%)                                  │
│  • Violation breakdown:                                                     │
│    - duration_exceeded:         7                                           │
│    - unsafe_exercise:           5                                           │
│    - missing_warmup:            0                                           │
...
│  ─────────────────────────────────────────────────────────────              │
│                                                                             │
│  Max exercises per workout:   [ 8  ]                                        │
│  Max workout duration (min):  [ 60 ]                                        │
...
### 5.9 Decision Receipt Audit

The audit log shows Ghost decisions with full context per Tech Spec §2.4:

```
┌─────────────────────────────────────────────────────────────────────────────┐
│  Decision Receipts                                           [Export CSV]  │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
...
│                                                                             │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │ ID                │ Time       │ User     │ Type       │ Outcome   │   │
│  ├───────────────────┼────────────┼──────────┼────────────┼───────────┤   │
│  │ DR-2026-01-28-001 │ 10:08:14   │ john@... │ TRANSFORM  │ Accepted  │   │
│  │ DR-2026-01-28-002 │ 09:45:22   │ jane@... │ SCHEDULE   │ Rejected  │   │
│  │ DR-2026-01-28-003 │ 09:12:08   │ bob@...  │ REMOVE     │ Accepted  │   │
│  │ DR-2026-01-28-004 │ 08:34:51   │ alice@.. │ TRANSFORM  │ Overridden│   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                                                             │
│  ─────────────────────────────────────────────────────────────              │
│                                                                             │
│  DECISION DETAIL: DR-2026-01-28-001                                         │
│                                                                             │
│  Type: CALENDAR_MUTATION (Block Transform)                                  │
│  User: john.doe@example.com                                                 │
│  Trust Phase: Scheduler (Phase 2)                                           │
│  Timestamp: 2026-01-28T10:08:14Z                                            │
│                                                                             │
│  INPUTS:                                                                    │
│  • Sleep last night: 5h 12m (below 6h baseline)                            │
│  • HRV: 32ms (crashed, -40% from personal baseline)                        │
│  • Calendar gap: 6:00-7:30 PM (detected as available)                      │
│  • Days since last workout: 3                                              │
│  • Scheduled block type: Heavy Lifts                                       │
│                                                                             │
│  DECISION: Transform block from "Heavy Lifts" → "Recovery Walk"           │
│                                                                             │
│  ALTERNATIVES CONSIDERED:                                                   │
│  1. Keep Heavy Lifts (rejected: injury risk 78%)                           │
│  2. Cancel block entirely (rejected: 3-day gap too long)                   │
│  3. Light mobility session (rejected: user prefers walks)                  │
│                                                                             │
│  CONFIDENCE: 89%                                                            │
│  TRUST IMPACT: +2 points if accepted, -5 if rejected                       │
│                                                                             │
│  OUTCOME: Accepted (user did not modify block)                              │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

### 5.10 Tier & Pricing Display

**Per PRD §2.4: Price for executive function, not utility.**
...
│  │                   │  │                   │  │                   │       │
│  │       $0          │  │    $49/month      │  │   Custom pricing  │       │
│  │                   │  │   or $499/year    │  │                   │       │
│  │                   │  │                   │  │                   │       │
│  │ • Observer phase  │  │ • All Ghost       │  │ • Everything in   │       │
│  │   only            │  │   phases          │  │   Premium         │       │
│  │ • Manual workout  │  │ • Full calendar   │  │ • API access      │       │
│  │   logging         │  │   autonomy        │  │ • Team management │       │
│  │ • Basic Phenome   │  │ • Auto-scheduling │  │ • Custom branding │       │
│  │ • Apple Watch     │  │ • Block transform │  │ • Dedicated       │       │
│  │   required        │  │ • Priority        │  │   support         │       │
│  │                   │  │   support         │  │ • SLA guarantees  │       │
│  │                   │  │                   │  │                   │       │
│  └───────────────────┘  └───────────────────┘  └───────────────────┘       │
│                                                                             │
│  Note: Apple Watch is REQUIRED for all tiers.                              │
│  The Ghost cannot operate without wrist-level sensor fidelity.             │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

### 5.11 Data Freshness Indicators

Admin dashboard must indicate when data is stale:

| Data Age  | Visual Indicator             | Meaning         |
| --------- | ---------------------------- | --------------- |
| < 1 min   | ● Green                      | Real-time       |
| 1-5 min   | ● Blue                       | Recent          |
| 5-30 min  | ● Yellow + timestamp         | Slightly stale  |
| > 30 min  | ● Orange + "Last updated: X" | Stale           |
| > 2 hours | ● Red + warning banner       | Action required |

### 5.12 Error States

When backend APIs fail, show clear error states:
...
│                           ⚠️                                                │
│                                                                             │
│                 Unable to load Ghost metrics                                │
│                                                                             │
│         The admin API is not responding.                                    │
│         Last successful connection: 10:08:14 AM                             │
│                                                                             │
│                     [Retry]    [View Logs]                                  │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
...
| 1.4     | January 28, 2026 | Design Team | Added Part V: Admin Dashboard specification with Ghost monitoring, Trust visibility, Decision Receipts, AI Pipeline config, pricing alignment ($49/month), admin access control model                                                                                                                    |
