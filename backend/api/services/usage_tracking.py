...
from database.sql_models import UserProfileDB, UserUsageLimitsDB, UserTierLimitsDB
...
class UsageTrackingService:
    ...
    async def check_user_limits(self, user_id: str) -> dict:
        ...
        user_result = await self.db.execute(
            select(UserProfileDB.user_tier, UserProfileDB.monthly_budget, UserProfileDB.current_month_usage)
        ...
        tier_result = await self.db.execute(
            select(UserTierLimitsDB)
            .where(UserTierLimitsDB.tier_name == user_tier)
        )
        ...
        if all(checks.values()):
            return {
                ...
                "remaining": {
                    ...
                    "weekly": tier_limits.weekly_limit - usage_data.weekly_requests_used,
                    "monthly": tier_limits.monthly_limit - usage_data.monthly_requests_used,
