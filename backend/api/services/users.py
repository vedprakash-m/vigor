...
from database.models import UserProfile, ProgressMetrics
import uuid
...
async def update_user_profile(db: Session, user_id: str, update_data: dict) -> UserProfile:
    """Update user profile."""
    user = db.query(UserProfileDB).filter(UserProfileDB.id == user_id).first()
    if not user:
        raise HTTPException(
...
async def create_progress_metric(db: Session, user_id: str, metric_data: dict) -> ProgressMetrics:
...
async def get_user_progress(db: Session, user_id: str, limit: int = 50) -> List[ProgressMetrics]:
    """Get user progress metrics."""
    metrics = db.query(ProgressMetricsDB).filter(
        ProgressMetricsDB.user_id == user_id
    ).order_by(ProgressMetricsDB.date.desc()).limit(limit).all()
