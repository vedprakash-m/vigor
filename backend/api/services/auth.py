...
async def register_user(db: Session, user_data: dict) -> UserProfile:
    ...
    if db.query(UserProfileDB).filter(UserProfileDB.email == user_data["email"]).first():
        raise HTTPException(
    ...
    if db.query(UserProfileDB).filter(UserProfileDB.username == user_data["username"]).first():
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
