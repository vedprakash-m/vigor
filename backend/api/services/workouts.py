...
from database.sql_models import WorkoutPlanDB, WorkoutLogDB
from database.models import WorkoutPlan, WorkoutLog, UserProfile, FitnessLevel
import uuid

async def create_workout_plan(db: Session, user_id: str, plan_data: dict) -> WorkoutPlan:
...
async def get_user_workout_plans(db: Session, user_id: str, limit: int = 50) -> List[WorkoutPlan]:
    """Get user's workout plans."""
    plans = db.query(WorkoutPlanDB).filter(
        WorkoutPlanDB.user_id == user_id
    ).order_by(WorkoutPlanDB.created_at.desc()).limit(limit).all()
...
async def get_workout_plan(db: Session, plan_id: str, user_id: str) -> WorkoutPlan:
    ...
    plan = db.query(WorkoutPlanDB).filter(
        WorkoutPlanDB.id == plan_id,
        WorkoutPlanDB.user_id == user_id
    ).first()
...
async def log_workout(db: Session, user_id: str, log_data: dict) -> WorkoutLog:
    ...
    plan = db.query(WorkoutPlanDB).filter(
        WorkoutPlanDB.id == log_data["plan_id"],
        WorkoutPlanDB.user_id == user_id
    ).first()
...
async def get_user_workout_logs(db: Session, user_id: str, limit: int = 50) -> List[WorkoutLog]:
    """Get user's workout logs."""
    logs = db.query(WorkoutLogDB).filter(
        WorkoutLogDB.user_id == user_id
    ).order_by(WorkoutLogDB.completed_at.desc()).limit(limit).all()
