...
class Badge(BaseModel):
    ...
    description: str
    icon: str
    category: str = Field(..., description="Badge category: streak, achievement, milestone, exploration")
...
def check_badge_unlocks(user_data: Dict) -> List[Badge]:
    ...
    for badge_id, badge_def in BADGE_DEFINITIONS.items():
        ...
        if 'workout_count' in requirements:
            should_unlock = user_data.get('workout_count', 0) >= requirements['workout_count']
        elif 'daily_streak' in requirements:
            should_unlock = user_data.get('daily_streak', {}).get('best', 0) >= requirements['daily_streak']
        elif 'ai_interactions' in requirements:
            should_unlock = user_data.get('ai_interactions', 0) >= requirements['ai_interactions']
        elif 'equipment_types' in requirements:
            should_unlock = len(user_data.get('equipment_types_used', [])) >= requirements['equipment_types']
        elif 'weekly_consistency' in requirements:
            should_unlock = user_data.get('weekly_consistency', 0) >= requirements['weekly_consistency']
...
async def get_gamification_stats(
    ...
    try:
        ...
        return GamificationStats(
            ...
            achievements=[Badge(**badge) for badge in user_data['badges'] if badge['category'] == 'achievement'],
...
async def get_leaderboard(
    ...
    return {
        ...
        'top_streaks': sorted(leaderboard_data, key=lambda x: x['daily_streak'], reverse=True)[:10],
        'top_badges': sorted(leaderboard_data, key=lambda x: x['badges_count'], reverse=True)[:10],
    }
