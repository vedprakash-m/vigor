    ...
    return {
        ...
        "tier_features": {
            ...
            "premium": {
                ...
                "monthly_limit": 1000,
                "monthly_budget": 25.0,
                "features": ["Advanced AI coaching", "Custom workouts", "Progress tracking", "Nutrition advice"]
            },
            "unlimited": {
                "daily_limit": 1000,
                "weekly_limit": 5000,
                ...
                "features": ["Unlimited AI coaching", "Premium workouts", "Advanced analytics", "Personal trainer mode"]
            }
        }
    }
...
async def get_usage_analytics(
    ...
    if limits_check.get("allowed"):
        ...
        usage_percentages = {
            "daily": max(0, 100 - (remaining.get("daily", 0) / 10 * 100)) if current_user.user_tier == "free" else 0,
            "weekly": max(0, 100 - (remaining.get("weekly", 0) / 50 * 100)) if current_user.user_tier == "free" else 0,
            "monthly": max(0, 100 - (remaining.get("monthly", 0) / 200 * 100)) if current_user.user_tier == "free" else 0,
            "budget": max(0, 100 - (remaining.get("budget", 0) / current_user.monthly_budget * 100))
        }
    else:
    ...
    return {
        "current_usage": {
            ...
            "monthly_budget": current_user.monthly_budget,
            "budget_remaining": current_user.monthly_budget - current_user.current_month_usage
        },
        "usage_percentages": usage_percentages,
        "limits_status": limits_check,
        "recommendations": {
            "should_upgrade": usage_percentages.get("monthly", 0) > 80,
            "next_tier": "premium" if current_user.user_tier == "free" else "unlimited" if current_user.user_tier == "premium" else None
        }
    }
