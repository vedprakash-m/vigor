...
async def get_llm_health_overview(
    ...
    avg_response_time = sum(model["response_time"] for model in MOCK_MODELS) / len(MOCK_MODELS)
    overall_error_rate = sum(model["error_rate"] * model["request_count"] for model in MOCK_MODELS) / total_requests
...
async def get_llm_model_health(
    model_id: str,
    current_user = Depends(require_admin)
):
...
async def trigger_failover(
    ...
    from_model = next((m for m in MOCK_MODELS if m["id"] == switch_request.from_model), None)
    to_model = next((m for m in MOCK_MODELS if m["id"] == switch_request.to_model), None)
...
async def get_historical_metrics(
    ...
    while current_time <= end_time:
        data_points.append({
            ...
            "response_time": 1000 + (hash(str(current_time)) % 500),  # Mock variation
