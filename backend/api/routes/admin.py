...
from database.models import UserProfile, AIProviderPriority, BudgetSettings, AIUsageLog
from database.sql_models import AIProviderPriorityDB, BudgetSettingsDB, AIUsageLogDB
from api.services.auth import get_current_user
from core.admin_llm_manager import CostCalculator, BudgetMonitor
...
async def verify_admin_user(current_user: UserProfile = Depends(get_current_user)) -> UserProfile:
...
async def get_ai_provider_priorities(
    ...
    priorities = db.query(AIProviderPriorityDB).order_by(AIProviderPriorityDB.priority).all()
...
async def create_ai_provider_priority(
    ...
    existing = db.query(AIProviderPriorityDB).filter(
...
async def update_ai_provider_priority(
    ...
    db_priority = db.query(AIProviderPriorityDB).filter(AIProviderPriorityDB.id == provider_id).first()
...
async def create_or_update_budget_settings(
    ...
    if existing_budget:
        ...
        existing_budget.alert_threshold_percentage = budget_data.alert_threshold_percentage
        existing_budget.auto_disable_on_budget_exceeded = budget_data.auto_disable_on_budget_exceeded
...
async def get_usage_statistics(
    ...
    weekly_requests = db.query(func.count(AIUsageLogDB.id)).filter(
    ...
    top_providers = db.query(
        ...
        desc(func.count(AIUsageLogDB.id))
    ).limit(5).all()
    
    # Recent usage logs
    recent_logs = db.query(AIUsageLogDB).order_by(
        desc(AIUsageLogDB.created_at)
    ).limit(10).all()
...
async def get_cost_breakdown(
    ...
    breakdown = db.query(
        ...
        AIUsageLogDB.created_at >= start_date,
        AIUsageLogDB.success == True
    ).group_by(
        AIUsageLogDB.provider_name,
        AIUsageLogDB.model_name
    ).order_by(
        desc(func.sum(AIUsageLogDB.cost))
    ).all()
    ...
    return {
        ...
        "breakdown": [{
            ...
            "avg_cost_per_request": float(item.total_cost / item.request_count) if item.request_count > 0 else 0,
