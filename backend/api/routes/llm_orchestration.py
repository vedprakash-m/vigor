...
from core.llm_orchestration import (
    LLMGateway, AdminConfigManager, KeyVaultClientService
)
...
from core.llm_orchestration.config_manager import (
    ModelConfiguration, ModelPriority, RoutingRule, ABTestConfiguration,
    BudgetConfiguration, BudgetResetPeriod
)
...
class LLMRequest(BaseModel):
    prompt: str = Field(..., min_length=1, max_length=50000, description="The prompt to send to the LLM")
    task_type: Optional[str] = Field(None, description="Type of task (chat, coding, analysis, etc.)")
...
class ModelConfigRequest(BaseModel):
    ...
    api_key_secret_identifier: str = Field(..., description="Key Vault secret identifier")
    key_vault_provider: str = Field(..., description="Key Vault provider (azure, aws, etc.)")
    priority: int = Field(3, ge=1, le=5, description="Model priority (1=highest, 5=lowest)")
...
class RoutingRuleRequest(BaseModel):
    ...
    target_models: List[str] = Field(..., description="Target model IDs in priority order")
...
class ABTestRequest(BaseModel):
    ...
    traffic_split: Dict[str, float] = Field(..., description="Traffic split between variants")
    model_variants: Dict[str, List[str]] = Field(..., description="Model variants for each test group")
    success_metrics: List[str] = Field(default_factory=list, description="Metrics to track")
...
class BudgetConfigRequest(BaseModel):
    ...
    reset_period: str = Field(..., description="Budget reset period (daily, weekly, monthly, quarterly)")
    alert_thresholds: List[float] = Field(default_factory=lambda: [0.5, 0.8, 0.95], description="Alert thresholds")
    auto_disable_at_limit: bool = Field(True, description="Auto-disable when limit reached")
    user_groups: List[str] = Field(default_factory=list, description="User groups (empty = global)")
...
class UsageReportRequest(BaseModel):
    ...
    group_by: Optional[str] = Field(None, description="Group by: user, model, provider, day")
...
@router.patch("/admin/models/{model_id}/toggle", dependencies=[Depends(get_current_admin_user)])
...
@router.get("/admin/analytics/usage-report", dependencies=[Depends(get_current_admin_user)])
        ...
        for model in all_models:
            ...
            model_info = {
                ...
                "health_status": {
                    ...
                    "latency_ms": health_status.latency_ms if health_status else None,
                    "last_check": health_status.last_check if health_status else None,
                    "error_message": health_status.error_message if health_status else None
                } if health_status else None
            }
