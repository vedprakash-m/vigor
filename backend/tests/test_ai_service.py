...
from api.services.ai import AIService, WorkoutGenerator, CoachChatService
from api.schemas.ai import WorkoutRequest, ChatRequest, WorkoutResponse, ChatResponse
...
class TestWorkoutGenerator:
    ...
    @pytest.mark.asyncio
    async def test_generate_workout_success(self, workout_generator, sample_workout_request, sample_user_profile):
        """Test successful workout generation"""
        with patch('api.services.ai.llm_gateway') as mock_gateway:
            # Mock LLM response
            mock_gateway.process_request = AsyncMock(return_value=MagicMock(
                content=json.dumps({
            ...
            result = await workout_generator.generate_workout(sample_workout_request, sample_user_profile)
    ...
    @pytest.mark.asyncio
    async def test_generate_workout_with_restrictions(self, workout_generator, sample_user_profile):
        ...
        with patch('api.services.ai.llm_gateway') as mock_gateway:
            ...
            result = await workout_generator.generate_workout(restricted_request, sample_user_profile)

            assert result.workout_name == "Low-Impact Cardio"
            assert "knee" in result.notes.lower() or "jumping" in result.notes.lower()

    @pytest.mark.asyncio
    async def test_generate_workout_llm_error(self, workout_generator, sample_workout_request, sample_user_profile):
        """Test workout generation when LLM service fails"""
        with patch('api.services.ai.llm_gateway') as mock_gateway:
            mock_gateway.process_request = AsyncMock(side_effect=Exception("LLM service unavailable"))

            with pytest.raises(Exception) as exc_info:
                await workout_generator.generate_workout(sample_workout_request, sample_user_profile)

            assert "LLM service unavailable" in str(exc_info.value)

    @pytest.mark.asyncio
    async def test_generate_workout_invalid_llm_response(self, workout_generator, sample_workout_request, sample_user_profile):
    ...
    @pytest.mark.asyncio
    async def test_personalize_workout_for_beginner(self, workout_generator, sample_user_profile):
        ...
        with patch('api.services.ai.llm_gateway') as mock_gateway:
            ...
            result = await workout_generator.generate_workout(beginner_request, sample_user_profile)

            assert result.difficulty_rating <= 3
            assert "beginner" in result.workout_name.lower() or "beginner" in result.notes.lower()
...
class TestCoachChatService:
    ...
    @pytest.mark.asyncio
    async def test_process_chat_fitness_question(self, coach_chat_service, sample_chat_request, sample_user_profile):
        ...
        with patch('api.services.ai.llm_gateway') as mock_gateway:
            ...
            result = await coach_chat_service.process_chat(sample_chat_request, sample_user_profile)
    ...
    @pytest.mark.asyncio
    async def test_process_chat_nutrition_question(self, coach_chat_service, sample_user_profile):
        ...
        with patch('api.services.ai.llm_gateway') as mock_gateway:
            ...
            result = await coach_chat_service.process_chat(nutrition_request, sample_user_profile)

            assert "nutrition" in result.response.lower() or "eat" in result.response.lower()
            assert result.context == "nutrition"

    @pytest.mark.asyncio
    async def test_process_chat_with_context_history(self, coach_chat_service, sample_user_profile):
        ...
        chat_with_history = ChatRequest(
            ...
            conversation_history=[
                ...
                {"role": "assistant", "content": "Great leg exercises include squats, lunges, and leg press..."}
            ]
        )
        ...
        with patch('api.services.ai.llm_gateway') as mock_gateway:
            ...
            result = await coach_chat_service.process_chat(chat_with_history, sample_user_profile)

            assert "knee" in result.response.lower()
            assert len(chat_with_history.conversation_history) == 2

    @pytest.mark.asyncio
    async def test_process_chat_inappropriate_content(self, coach_chat_service, sample_user_profile):
        ...
        with patch('api.services.ai.llm_gateway') as mock_gateway:
            ...
            result = await coach_chat_service.process_chat(inappropriate_request, sample_user_profile)

            assert "fitness" in result.response.lower()
            assert "tax" in result.response.lower()

    @pytest.mark.asyncio
    async def test_process_chat_error_handling(self, coach_chat_service, sample_chat_request, sample_user_profile):
        """Test error handling in chat processing"""
        with patch('api.services.ai.llm_gateway') as mock_gateway:
            mock_gateway.process_request = AsyncMock(side_effect=Exception("Chat service error"))

            with pytest.raises(Exception) as exc_info:
                await coach_chat_service.process_chat(sample_chat_request, sample_user_profile)

            assert "Chat service error" in str(exc_info.value)

    @pytest.mark.asyncio
    async def test_process_chat_with_user_preferences(self, coach_chat_service, sample_user_profile):
        ...
        with patch('api.services.ai.llm_gateway') as mock_gateway:
            ...
            result = await coach_chat_service.process_chat(equipment_request, sample_user_profile)
...
class TestAIServiceIntegration:
    ...
    async def test_workout_generation_and_chat_integration(self, sample_user_profile):
        ...
        with patch('api.services.ai.llm_gateway') as mock_gateway:
            ...
            workout = await ai_service.generate_workout(workout_request, sample_user_profile)
            ...
            chat_response = await ai_service.process_chat(chat_request, sample_user_profile)
    ...
    async def test_ai_service_rate_limiting(self, sample_user_profile):
        ...
        with patch('api.services.ai.llm_gateway') as mock_gateway:
            ...
            mock_gateway.process_request = AsyncMock(side_effect=Exception("Rate limit exceeded"))
    ...
    async def test_ai_service_user_tier_handling(self, sample_user_profile):
        ...
        with patch('api.services.ai.llm_gateway') as mock_gateway:
            ...
            result = await ai_service.generate_workout(premium_request, sample_user_profile)
