...
from api.services import auth, ai, usage_tracking, users, workouts
...
class TestAuthService:
    ...
    def test_auth_service_methods(self):
        ...
        auth_items = dir(auth)
        auth_methods = [item for item in auth_items if 'authenticate' in item.lower() or 'login' in item.lower() or 'register' in item.lower()]
...
class TestAIService:
    ...
    def test_ai_service_functions(self):
        ...
        functions = [item for item in dir(ai) if callable(getattr(ai, item)) and not item.startswith('_') and item[0].islower()]
...
class TestServiceConfiguration:
    ...
    def test_service_utilities(self):
        ...
        for module in [auth, ai, usage_tracking, users, workouts]:
            ...
            utilities = [item for item in dir(module)
