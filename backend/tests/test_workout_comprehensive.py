...
from database.models import Exercise, FitnessLevel, WorkoutLog, WorkoutPlan
...
class TestWorkoutPlanCreation:
    ...
    def test_create_workout_plan_authenticated(
        self, client, sample_workout_plan_data, mock_current_user
    ):
    ...
    def test_create_workout_plan_unauthenticated(
        self, client, sample_workout_plan_data
    ):
        """Test creating a workout plan without authentication"""
        response = client.post("/workouts/plans", json=sample_workout_plan_data)
        assert response.status_code in [401, 403, 404]

    def test_create_workout_plan_invalid_data(self, client):
        """Test creating a workout plan with invalid data"""
        invalid_data = {"name": "", "exercises": []}  # Empty name  # Empty exercises
        response = client.post(
            "/workouts/plans",
            json=invalid_data,
            headers={"Authorization": "Bearer test-token"},
...
class TestWorkoutPlanRetrieval:
    ...
    def test_get_user_workout_plans(self, client, mock_current_user):
        ...
        with patch(
            ...
            response = client.get(
                "/workouts/plans", headers={"Authorization": "Bearer test-token"}
            )
...
class TestWorkoutLogging:
    ...
    def test_get_workout_logs(self, client, mock_current_user):
        ...
        with patch(
            ...
            response = client.get(
                "/workouts/logs", headers={"Authorization": "Bearer test-token"}
            )
            assert response.status_code in [200, 401, 404]
...
class TestWorkoutDays:
    ...
    def test_get_workout_days(self, client, mock_current_user):
        ...
        with patch(
            ...
            response = client.get(
                "/workouts/days", headers={"Authorization": "Bearer test-token"}
            )
            assert response.status_code in [200, 401, 404]
...
class TestWorkoutServiceUnit:
    ...
    async def test_create_workout_plan_service(self, db_session):
        ...
        class MockExercise:
            def dict(self):
                return {"name": "Push-ups", "sets": 3, "reps": 15}
