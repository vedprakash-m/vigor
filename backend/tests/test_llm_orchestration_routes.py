...
from api.schemas.admin import ABTestRequest, ModelConfigRequest
...
class TestLLMOrchestrationRoutes:
    ...
    def test_chat_completion_success(
        self, mock_gateway, mock_get_user, client, auth_headers, mock_gateway_response
    ):
    ...
    def test_chat_completion_invalid_request(
        self, mock_gateway, mock_get_user, client, auth_headers
    ):
        ...
        response = client.post(
            "/api/llm/chat/completions", json={"max_tokens": 100}, headers=auth_headers
        )
    ...
    @patch("api.routes.llm_orchestration.get_current_user")
    @patch("api.routes.llm_orchestration.llm_gateway")
    def test_chat_completion_gateway_error(
        self, mock_gateway, mock_get_user, client, auth_headers
    ):
    ...
    def test_streaming_chat_completion(
        ...
        response = client.post(
            "/api/llm/chat/completions",
            json={"prompt": "Say hello world", "max_tokens": 10, "stream": True},
            headers=auth_headers,
        )
    ...
    @patch("api.routes.llm_orchestration.get_current_user")
    @patch("api.routes.llm_orchestration.llm_gateway")
    def test_provider_status_admin(
        self, mock_gateway, mock_get_user, client, admin_headers
    ):
        ...
        mock_gateway.get_provider_status = AsyncMock(
            return_value={
                "total_models": 3,
                "healthy_models": 2,
                "adapters": {
        ...
        response = client.get("/api/llm/providers/status", headers=admin_headers)
    ...
    def test_provider_status_non_admin(self, mock_get_user, client, auth_headers):
        ...
        response = client.get("/api/llm/providers/status", headers=auth_headers)

        assert response.status_code == 403  # Forbidden

    @patch("api.routes.llm_orchestration.get_current_user")
    @patch("api.routes.llm_orchestration.admin_config_manager")
    def test_model_configuration_create(
        self, mock_config_manager, mock_get_user, client, admin_headers
    ):
        ...
        response = client.post(
            "/api/llm/models/configure", json=config_data, headers=admin_headers
        )
    ...
    def test_model_configuration_list(
        ...
        response = client.get("/api/llm/models", headers=admin_headers)
    ...
    @patch("api.routes.llm_orchestration.get_current_user")
    @patch("api.routes.llm_orchestration.admin_config_manager")
    def test_model_configuration_update(
        self, mock_config_manager, mock_get_user, client, admin_headers
    ):
        ...
        update_data = {"priority": 3, "is_active": False}

        response = client.put(
            "/api/llm/models/gpt-3.5-turbo", json=update_data, headers=admin_headers
        )
    ...
    @patch("api.routes.llm_orchestration.get_current_user")
    @patch("api.routes.llm_orchestration.admin_config_manager")
    def test_model_configuration_delete(
        self, mock_config_manager, mock_get_user, client, admin_headers
    ):
        ...
        response = client.delete("/api/llm/models/old-model", headers=admin_headers)

        assert response.status_code == 204
        mock_config_manager.remove_model_configuration.assert_called_once_with(
            "old-model"
        )
    ...
    def test_usage_analytics_admin(self, mock_get_user, client, admin_headers):
        ...
        with patch("api.routes.llm_orchestration.usage_analytics") as mock_analytics:
            mock_analytics.get_usage_summary = AsyncMock(
                return_value={
                    "total_requests": 1500,
                    "total_tokens": 50000,
                    "total_cost": 12.50,
            ...
            response = client.get("/api/llm/analytics/usage", headers=admin_headers)
    ...
    @patch("api.routes.llm_orchestration.get_current_user")
    @patch("api.routes.llm_orchestration.llm_gateway")
    def test_ab_test_configuration(
        self, mock_gateway, mock_get_user, client, admin_headers
    ):
        ...
        with patch("api.routes.llm_orchestration.ab_test_manager") as mock_ab_manager:
            mock_ab_manager.create_test = AsyncMock(
                return_value={"test_id": "test-123"}
            )

            response = client.post(
                "/api/llm/ab-tests", json=ab_test_data, headers=admin_headers
            )
    ...
    def test_rate_limiting(self, mock_get_user, client, auth_headers):
        ...
        response = client.post(
            "/api/llm/chat/completions",
            json={"prompt": "Test rate limiting", "max_tokens": 10},
            headers=auth_headers,
        )
    ...
    def test_request_validation_edge_cases(
        ...
        response = client.post(
            "/api/llm/chat/completions",
            json={"prompt": "Test prompt", "max_tokens": -1},
            headers=auth_headers,
        )
