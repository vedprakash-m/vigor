...
class TestAICoachChat:
    ...
    def test_chat_endpoint_authenticated(self, client, mock_current_user):
        ...
        with patch('core.security.get_current_active_user', return_value=mock_current_user):
            with patch('api.services.ai.chat_with_ai_coach', new_callable=AsyncMock) as mock_chat:
...
class TestAIWorkoutGeneration:
    ...
    def test_generate_workout_authenticated(self, client, mock_current_user):
        ...
        with patch('core.security.get_current_active_user', return_value=mock_current_user):
            with patch('api.services.ai.generate_ai_workout_plan', new_callable=AsyncMock) as mock_gen:
...
class TestAIServiceUnit:
    ...
    async def test_chat_with_ai_coach_direct(self, db_session, mock_current_user):
        ...
        with patch('api.services.ai.direct_get_ai_coach_response', new_callable=AsyncMock) as mock_response:
    ...
    async def test_generate_ai_workout_plan_direct(self, db_session, mock_current_user):
        ...
        with patch('api.services.ai.direct_generate_workout_plan', new_callable=AsyncMock) as mock_gen:
            mock_gen.return_value = {
...
class TestAIServiceErrorHandling:
    ...
    def test_ai_timeout_handling(self, client, mock_current_user):
        ...
        with patch('core.security.get_current_active_user', return_value=mock_current_user):
            with patch('api.services.ai.chat_with_ai_coach', new_callable=AsyncMock) as mock_chat:
                mock_chat.side_effect = TimeoutError("AI service timeout")
...
class TestAIFunctionsClient:
    ...
    async def test_functions_client_coach_chat(self):
        ...
        with patch('core.function_client.FunctionsClient._call_function', new_callable=AsyncMock) as mock_call:
            mock_call.return_value = {"response": "AI response"}
            
            client = FunctionsClient()
            
            try:
                result = await client.coach_chat(
                    message="Hello",
    ...
    async def test_functions_client_generate_workout(self):
        ...
        with patch('core.function_client.FunctionsClient._call_function', new_callable=AsyncMock) as mock_call:
            ...
            try:
                result = await client.generate_workout_plan(
                    fitness_level="intermediate",
                    goals=["strength"],
                    equipment="full_gym",
                    duration_minutes=45
