...
class FunctionPerformanceMonitor:
    ...
    async def monitor_call(self, function_name: str, call_func: Callable, *args: Any, **kwargs: Any) -> Any:
        """
        ...
        try:
            ...
            avg_time = self.call_metrics[function_name]["total_time"] / self.call_metrics[function_name]["count"]
            logger.info(f"Function {function_name} call completed in {duration:.2f}s (avg: {avg_time:.2f}s)")
            ...
            end_time = time.time()
            duration = end_time - start_time
            logger.error(f"Function {function_name} failed after {duration:.2f}s: {str(e)}")
            raise

    async def keep_warm(self, warmup_func: Callable, function_name: str, interval: Optional[int] = None) -> None:
        """
        ...
        while True:
            ...
            last_call = self.call_metrics.get(function_name, {}).get("last_call_time", 0)
    ...
    def get_metrics(self, function_name: Optional[str] = None) -> Dict[str, Any]:
        ...
        if function_name:
            if function_name in self.call_metrics:
                ...
                metrics["avg_time"] = metrics["total_time"] / metrics["count"] if metrics["count"] > 0 else 0
                return metrics
            return {}
        ...
        for name, metrics in self.call_metrics.items():
            ...
            func_metrics["avg_time"] = func_metrics["total_time"] / func_metrics["count"] if func_metrics["count"] > 0 else 0
