...
class SecretReference:
    ...
    secret_identifier: str  # e.g., "arn:aws:secretsmanager:...", "https://vault.azure.com/secrets/..."
...
class BaseKeyVaultClient(ABC):
    ...
    @abstractmethod
    async def get_secret(self, secret_identifier: str, version: Optional[str] = None) -> str:
...
class AWSSecretsManagerClient(BaseKeyVaultClient):
    ...
    async def get_secret(self, secret_identifier: str, version: Optional[str] = None) -> str:
        ...
        try:
            ...
            logger.info(f"Retrieving secret from AWS Secrets Manager: {secret_identifier}")
    ...
    async def get_secret(self, secret_ref: SecretReference) -> str:
        ...
        try:
            ...
            if secret_ref.provider not in self._clients:
                raise ValueError(f"No client registered for provider: {secret_ref.provider.value}")
            
            client = self._clients[secret_ref.provider]
            
            # Retrieve secret
            secret_value = await client.get_secret(
                secret_ref.secret_identifier, 
                secret_ref.version
            )
            ...
            logger.info(f"Successfully retrieved secret: {secret_ref.secret_identifier}")
            return secret_value
            
        except Exception as e:
            logger.error(f"Failed to retrieve secret {secret_ref.secret_identifier}: {e}")
            raise
    ...
    try:
        ...
        if provider == "azure":
            ...
            if vault_url:
                ...
                key_vault_service.register_client(KeyVaultProvider.AZURE_KEY_VAULT, client)
        
        elif provider == "aws":
            region = os.getenv("AWS_REGION", "us-east-1")
            client = AWSSecretsManagerClient(region)
            key_vault_service.register_client(KeyVaultProvider.AWS_SECRETS_MANAGER, client)
            ...
            if vault_url and vault_token:
                ...
                key_vault_service.register_client(KeyVaultProvider.HASHICORP_VAULT, client)
