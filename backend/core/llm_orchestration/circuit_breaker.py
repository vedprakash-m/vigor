...
class CircuitBreakerManager:
    ...
    def add_model(self, model_id: str, config: CircuitBreakerConfig = None):
    ...
    def can_proceed(self, model_id: str) -> bool:
        ...
        if circuit.state == CircuitState.CLOSED:
            ...
            if current_time - circuit.last_failure_time >= circuit.config.recovery_timeout:
    ...
    def record_failure(self, model_id: str):
        ...
        if circuit.state == CircuitState.HALF_OPEN:
            ...
            if circuit.failure_count >= circuit.config.failure_threshold:
                circuit.state = CircuitState.OPEN
                logger.warning(f"Circuit breaker for {model_id} OPENED due to {circuit.failure_count} failures")
