...
class ABTestConfiguration:
    ...
    model_variants: Dict[str, List[str]]  # {"variant_a": ["gpt-4"], "variant_b": ["gemini-pro"]}
...
class AdminConfigManager:
    ...
    def get_models_by_priority(self, priority: Optional[ModelPriority] = None) -> List[ModelConfiguration]:
    ...
    def get_budget_for_user_group(self, user_groups: List[str]) -> Optional[BudgetConfiguration]:
    ...
    def export_configuration(self) -> Dict[str, Any]:
        ...
        return {
            "models": {k: self._serialize_model_config(v) for k, v in self.models.items()},
            "routing_rules": {k: self._serialize_routing_rule(v) for k, v in self.routing_rules.items()},
            "ab_tests": {k: self._serialize_ab_test(v) for k, v in self.ab_tests.items()},
            "budgets": {k: self._serialize_budget_config(v) for k, v in self.budgets.items()},
            "user_tiers": {k: self._serialize_user_tier(v) for k, v in self.user_tiers.items()},
            "caching_config": self._serialize_caching_config(self.caching_config),
            "rate_limit_config": self._serialize_rate_limit_config(self.rate_limit_config)
        }
    ...
    def _serialize_rate_limit_config(self, config: RateLimitConfiguration) -> Dict[str, Any]:
