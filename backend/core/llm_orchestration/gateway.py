...
from .config_manager import AdminConfigManager, ModelConfiguration, UserTierConfiguration
from .adapters import (
    LLMServiceAdapter, AdapterFactory, LLMRequest, LLMResponse, 
    create_adapters_from_configs, health_check_all_adapters
)
...
class LLMGateway:
    ...
    async def initialize(self):
        ...
        try:
            ...
            if not active_models:
                ...
                from .key_vault import SecretReference, KeyVaultProvider
    ...
    async def process_request(self, request: GatewayRequest) -> GatewayResponse:
        ...
        try:
            ...
            llm_response = await self._execute_llm_request(selected_adapter, enriched_request)
            ...
            fallback_response = await self._handle_error_fallback(request, request_id, str(e))
    ...
    async def process_stream(self, request: GatewayRequest) -> AsyncGenerator[str, None]:
        ...
        try:
            logger.info(f"Processing streaming request {request_id} for user {request.user_id}")
            ...
            mock_response = LLMResponse(
                ...
                cost_estimate=selected_adapter.estimate_cost(request.prompt, len(full_response.split())),
                latency_ms=0,
    ...
    async def admin_toggle_model(self, model_id: str, is_active: bool) -> bool:
        ...
        try:
            success = await self.config_manager.toggle_model_activation(model_id, is_active)
    ...
    async def _enrich_request(self, request: GatewayRequest, request_id: str) -> LLMRequest:
    ...
    async def _select_model(self, request: LLMRequest) -> LLMServiceAdapter:
        ...
        selected_model_id = await self.routing_engine.select_model(context, list(self.adapters.keys()))
    ...
    async def _select_fallback_model(self, request: LLMRequest) -> LLMServiceAdapter:
        ...
        for adapter in self.adapters.values():
            if adapter.is_healthy() and self.circuit_breaker.can_proceed(adapter.model_id):
                return adapter
    ...
    async def _execute_llm_request(self, adapter: LLMServiceAdapter, request: LLMRequest) -> LLMResponse:
    ...
    async def _log_usage(self, request: GatewayRequest, response: LLMResponse, request_id: str):
    ...
    async def _handle_error_fallback(
        ...
        try:
            if "fallback" in self.adapters:
                ...
                fallback_response = await fallback_adapter.generate_response(llm_request)
        ...
        try:
            ...
            for model_id, result in health_results.items():
                if not result.is_healthy:
                    logger.warning(f"Model {model_id} health check failed: {result.error_message}")
