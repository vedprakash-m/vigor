...
def upgrade():
    ...
    op.create_index(
        "idx_provider_priority", "ai_provider_priorities", ["priority", "is_enabled"]
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "alert_threshold_percentage", sa.Float(), nullable=False, default=80.0
        ),
        sa.Column(
            "auto_disable_on_budget_exceeded",
            sa.Boolean(),
            nullable=False,
            default=True,
        ),
    ...
    op.create_index(
        "idx_usage_date_provider", "ai_usage_logs", ["created_at", "provider_name"]
    )
    op.create_index(
